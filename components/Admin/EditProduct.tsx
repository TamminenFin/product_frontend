...
import {
  useGetProductSDetailsWithSallerInfo,
  useUpdateProduct,
...
const EditProduct = () => {
  const { id } = useParams();
  const { data, isLoading } = useGetProductSDetailsWithSallerInfo(id as string);
  ...
  const categoryOption =
    data?.data?.sallerId?.categories?.map((cat: TCategory) => ({
      label: cat.name,
      value: cat.name,
    })) || [];
