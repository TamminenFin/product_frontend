...
export default function EditSallerInfo() {
  const { id } = useParams();
  const { mutate, isPending } = useUpdateSallerProfileByAdmin();
  const { data: city, isLoading: isCityLoading } = useGetAllCity();

  const { data, isLoading } = useGetCurrentSaller((id as string) ?? "", {
    enabled: !!id || !!city,
  });
